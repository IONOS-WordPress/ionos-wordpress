...
test.describe('essentials:dashboard deep-links block', () => {
  test.beforeAll(async () =>
    execSync(
      'pnpm -s run wp-env run tests-cli wp post delete $(pnpm -s run wp-env run tests-cli wp post list --format=ids --post_status=auto-draft,publish --post_type=custom_dashboard,wp_global_styles) --force ||:'
    )
  );
  ...
  test('contains output for valid tenant "ionos"', async ({ admin, editor }) => {
    ...
    await expect(deepLinksBlock.locator('css=.wp-block-group > div')).not.toHaveCount(0);
  });
