  ...
  <div id="ionos_next_best_actions__all_done">
   <div class="container">
    ...
    <?php if ('ionos' === Tenant::get_slug()) { ?>
      <div class="buttons">
        <a href="<?php echo \esc_url($digital_guide_url); ?>" class="button button--secondary" target="_blank"><?php echo esc_html__('View Digital Guide', 'ionos-essentials')?></a>
        <?php if ($survey_dismissed) { ?>
          <a href="<?php echo \esc_url(get_survey_url()); ?>" class="button button--secondary" target="_blank"><?php echo esc_html__('Leave feedback', 'ionos-essentials')?></a>
        <?php } ?>
      </div>
    <?php } ?>
   </div>
