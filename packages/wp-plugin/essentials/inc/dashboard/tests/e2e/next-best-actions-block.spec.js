...
test.describe('essentials:dashboard next-best-actions block', () => {
  ...
  test('test dismissing an option ', async ({ admin, page }) => {
    // show dashboard and click on dismiss button of "addSiteDescription" action
    ...
    let dismissAncor = await iframeBodyLocator.locator('css=.dismiss-nba[data-nba-id="addSiteDescription"]');
    await expect(dismissAncor).toHaveCount(1);
    await dismissAncor.click();

    // show dashboard and ensure "addSiteDescription" action is not more available
    await admin.visitAdminPage('/');
    iframeLocator = await page.locator('iframe');
    iframeBodyLocator = await iframeLocator.contentFrame().locator('body');
    dismissAncor = await iframeBodyLocator.locator('css=.dismiss-nba[data-nba-id="addSiteDescription"]');
