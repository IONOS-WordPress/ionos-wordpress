...
/*
  WORKAROUND for the ephemeral PHPUnit test container :
  wp-cli commands run against the tests-WordPress-PHPUnit database but not against
  the same PHP process serving requests. Therefore, when the option is changed via
  wp-cli, the request-serving process does not reflect the change
  (=> create/delete WP_CONTENT_DIR/object-cache.php).
  this workaround ensures that on init, the request-serving process checks the option
  value and syncs the state (=> create/delete WP_CONTENT_DIR/object-cache.php).
*/
if (! defined('WP_CLI') && getenv('WP_TESTS_DIR')!== false && in_array(wp_get_environment_type(), ['local'], true)) {
  \add_action('init', function () {
    ...
    if ($enabled && ! file_exists(OBJECT_CACHE_PATH)) {
      enable_cache();
    } elseif (! $enabled && file_exists(OBJECT_CACHE_PATH)) {
      disable_cache();
    }
  });
}
