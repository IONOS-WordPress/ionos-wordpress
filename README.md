...
- automatic provisioning of a Docker-based WordPress dev container (`wp-alpine`, see `packages/docker/wp-alpine/`) with WordPress autocompletion and PHP debugging (Xdebug) enabled out of the box
...
- `pnpm start` : starts the persistent `wp-alpine` dev container (building its Docker image first if needed), dynamically bind-mounting all WordPress plugins/themes/mu-plugins declared in the repository.

  Xdebug is baked into the `wp-alpine` image and enabled by default - no separate flag needed. It attaches on every request and connects back to the IDE on `host.docker.internal:9003` (mapped to the docker host via `--add-host`), so simply starting the generated `ionos-wordpress` launch configuration in vscode is enough to hit breakpoints.
...
- `pnpm stop` : stops the `wp-alpine` dev container.

- `pnpm destroy` : removes the `wp-alpine` dev container and its per-stack overlay data (the shared, version-keyed WordPress core cache survives).
...
- `.vscode/launch.json` contains the Xdebug launch configuration for `vscode`
  ...
  It is generated by `packages/docker/wp-alpine/scripts/generate-vscode-launch.sh` **on every `pnpm start`**, so its path mappings always match the plugins/mu-plugins/themes currently bind-mounted into the `wp-alpine` container (plus the phpunit mappings used by `pnpm test:php`).
