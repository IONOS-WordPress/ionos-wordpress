...
- **maintenance mode** - `WP_Upgrader::maintenance_mode()` keeps visitors off a half-written installation. Note this is called by the _callers_ of `run()`, not by `run()` itself, so it has to be invoked explicitly.
