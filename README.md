...
- `.vscode/launch.json` contains the Xdebug launch configuration for `vscode`
  ...
  It is generated by `packages/docker/wordpress-alpine/scripts/_generate-vscode-launch.sh` **on every `pnpm start`**, so its path mappings always match the plugins/mu-plugins/themes currently bind-mounted into the `wordpress-alpine` container (plus the phpunit mappings used by `pnpm test:php`).
