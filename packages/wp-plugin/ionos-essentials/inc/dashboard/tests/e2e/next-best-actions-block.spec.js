...
test.describe('essentials:dashboard next-best-actions block', () => {
  test.beforeAll(async () => {
    ...
    // avoid the redirect leading to the switch page. done for redirect_count > 3
  ...
  test('test dismissing an option ', async ({ admin, page }) => {
    ...
    let dismissAncor = iframeBodyLocator.locator('css=.dismiss-nba[data-nba-id="edit-and-complete"]');
    ...
    dismissAncor = iframeBodyLocator.locator('css=.dismiss-nba[data-nba-id="edit-and-complete"]');
