...
function ionos.wordpress.mark_added_deepl_translations() {
  ...
  local msg_ids_translated_by_deepl=()
  local readonly GENERATED_BY_DEEPL_COMMENT="# generated by deepl"
  # loop over all uncommitted lines starting with 'msgstr' in the PO file
  while IFS= read -r added_deepl_translation; do
    # get the matching msgid line
    local msg_id=$(grep -B1 -F "$added_deepl_translation" "$PO_FILE" | head -1)
    # get the (comment) line before the msgid
    local msg_comment=$(grep -B1 -F "$msg_id" "$PO_FILE" | head -1)

    # if comment before msgid does not start with the generated by deepl comment
    if [[ ! "$msg_comment" == "$GENERATED_BY_DEEPL_COMMENT"* ]]; then
      # remember the msg_id as translated by deepl
      msg_ids_translated_by_deepl+=("$msg_id")
    fi
  done < <(git diff --no-color $PO_FILE | grep '^+msgstr ".*$' | sed 's/^+//')

  # replace the msgid in the PO file with the new msg_id
  # and add the generated by deepl comment
  for msg_id in "${msg_ids_translated_by_deepl[@]}"; do
    # escape slashes and special characters for sed
    local escaped_msg_id=$(printf '%s\n' "$msg_id" | sed 's/[\/&]/\\&/g')
    local escaped_replacement=$(printf '%s\n' "$GENERATED_BY_DEEPL_COMMENT\n$msg_id" | sed ':a;N;$!ba;s/\n/\\n/g; s/[\/&]/\\&/g')
    sed -i "s/$escaped_msg_id/$escaped_replacement/" "$PO_FILE"
  done
}
...
function ionos.wordpress.dennis() {
  if [[ "$FIX" == 'yes' ]] && [[ "${USE[@]}" =~ i18n ]]; then
    ...
    for PO_FILE in $(find ./packages -maxdepth 5 -mindepth 4 -type f -path '*/languages/*.po'); do
      ...
      if [[ "0" == "$diff_error_code" ]]; then
        git checkout $PO_FILE
      else
        # mark the deepl translated entries
